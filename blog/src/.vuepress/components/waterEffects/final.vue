...
const vertexSrc = `
 ...
 const fragmentSrc = `
        ...
        fg.rgb *= smoothstep(-0.1, 1.5, uvs.y * colorStepUv * color.rgb);
        fg.rgb = mix(fg.rgb, vec3(1., 1., 1.) , smoothstep(0.68, 0.89 ,1.0 - uvs.y)).rgb;
...
onMounted(() => {
    const canvas = document.getElementById('final-canvas-container');
    if (canvas) {
